...
本项目是 `xzplays/babylon-ggec` 的程序化 Babylon.js 园区展示系统，目标是交付一个可离线构建、可浏览、可进入建筑的一比一现实比例厂区/园区场景。当前交互定位是园区导览式 FPS 漫游：默认步行速度较慢、第一视角稳定眼高、第三/俯瞰视角显示低模方块人访客。
...
- `WASD` / 方向键：按当前视角方向移动。
- `Shift`：慢速加速，默认从 `1.25 m/s` 提升到约 `2.25 m/s`。
