...
本项目是 `xzplays/babylon-ggec` 的程序化 Babylon.js 园区展示系统，目标是交付一个可离线构建、可浏览、可进入建筑的一比一现实比例厂区/园区场景。当前交互定位是园区导览式 FPS 漫游：默认步行速度较慢、但速度滑块可扩展到快速巡园；第一视角稳定眼高，第三/俯瞰视角显示低模方块人访客。

## 比例规则

- 全局约定：`1 Babylon unit = 1 real-world meter`。
- 园区边界：`1200m × 900m`，位于 `src/data/campusLayout.ts` 的 `campusBounds`。
- 建筑、道路、门洞、楼层、停车位、围墙、人物高度和移动速度均按米制数据配置；移动速度单位为 `m/s`。
...
- 公共材质集中复用，树木、路灯、车位线、窗户、行人、车辆、停车场车辆、导览牌、垃圾桶、长椅、货物和叉车等重复元素不再为每个对象创建独立材质。

## 操作方式

- `WASD` / 方向键：按当前视角方向移动。
- 速度滑块：可在 `1 - 20 m/s` 范围内调节，默认 `1.25 m/s` 适合慢速导览，较高速度适合调试/快速巡园。
- `Shift`：保持当前滑块速度（不再额外加速，避免叠加过快）。
- `F1`：第一视角；`F2`：第三视角；`F4`：俯瞰视角。
- `E`：靠近建筑入口标记时进入一楼。
- `F`：在电梯附近切换到下一楼层。
- 右侧建筑面板：按区域分组，可“到门前”或“进入一楼”。
- `R` 或 HUD“重置到主入口”按钮：从卡住或误入位置恢复。
- HUD 显示当前视角、区域/建筑/楼层、坐标、移动速度 `m/s` 和操作提示。
- 第三视角和俯瞰视角显示由头、身体、手、腿和正面标记组成的低模方块人，用于判断当前朝向。
...
src/player/playerController.ts        # 人物移动、deltaTime 秒级速度、进门、电梯交互
src/player/createBlockAvatar.ts       # 低模方块人访客模型
src/player/playerConstants.ts          # 玩家高度、眼高、速度和楼层高度常量
