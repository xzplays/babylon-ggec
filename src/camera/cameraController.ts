...
export const createCameraController=(scene:Scene,canvas:HTMLCanvasElement,player:any)=>{
 ...
 const setViewMode=(m:ViewMode)=>{if(viewMode==="first")player.position.copyFrom(first.position).addInPlaceFromFloats(0,-EYE_HEIGHT+PLAYER_HEIGHT/2,0); if(m==="first")first.position.copyFrom(player.position).addInPlaceFromFloats(0,EYE_HEIGHT-PLAYER_HEIGHT/2,0); viewMode=m; third.radius=m==="third"?Math.min(third.radius,12):third.radius; scene.activeCamera=m==="first"?first:m==="third"?third:overhead; syncTargets();};
 const syncTo=(p:Vector3,yaw=0,indoor=false)=>{player.position.copyFrom(p);first.position.copyFrom(p).addInPlaceFromFloats(0,EYE_HEIGHT-PLAYER_HEIGHT/2,0);first.rotation=new Vector3(0,yaw,0);if(indoor)third.radius=4;syncTargets();};
 return {first,third,overhead,get viewMode(){return viewMode},setViewMode,syncTo,syncTargets};
};
