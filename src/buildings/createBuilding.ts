...
export const createBuilding=(scene:Scene,spec:BuildingSpec,m:CampusMaterials)=>{
 ...
 add("ground-floor",new Vector3(x,.08,z),new Vector3(w,.16,d),m.floor,false);
 for(let f=1;f<spec.floors;f++) add(`level-${f+1}-slab`,new Vector3(x,f*fh,z),new Vector3(w-1.2,.18,d-1.2),m.floor,true);
 add("roof",new Vector3(x,h+.25,z),new Vector3(w+1.4,.5,d+1.4),m.roof,true);
 ...
 if(spec.type==="office"||spec.type==="research"){add("lobby-desk",new Vector3(x,.55,z+d*.22),new Vector3(9,1.1,1.4),m.furniture,true); for(let i=-2;i<=2;i++) add(`room-partition-${i}`,new Vector3(x+i*w/6,1.8,z),new Vector3(.25,3.2,d*.55),m.innerWall,true);}
