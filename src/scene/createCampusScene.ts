...
export const createCampusScene=(scene:Scene)=>{scene.clearColor=new Color4(.62,.82,1,1);scene.collisionsEnabled=true;scene.gravity=new Vector3(0,-.35,0);new HemisphericLight("现实尺度天空光",new Vector3(.4,1,.2),scene).intensity=.82;const m=createCampusMaterials(scene);
 const ground=MeshBuilder.CreateGround("1200m x 900m 园区地面",{width:campusBounds.width,height:campusBounds.depth},scene);ground.material=m.grass;applyMeshMetadata(ground,{collides:true,walkable:true,blocksCamera:false});
 roads.forEach(r=>box(scene,r.name,new Vector3(r.x,.035,r.z),new Vector3(r.width,.07,r.depth),m.road,true,true,false));
 ...
 const plaza=MeshBuilder.CreateCylinder("中央景观广场 直径70m",{diameter:70,height:.12,tessellation:64},scene);plaza.position.y=.08;plaza.material=m.path;applyMeshMetadata(plaza,{collides:true,walkable:true,blocksCamera:false}); const lake=MeshBuilder.CreateCylinder("景观湖 120m x 42m",{diameter:1,height:.08,tessellation:64},scene);lake.position=new Vector3(155,.1,325);lake.scaling=new Vector3(60,1,21);lake.material=m.water;createLabel(scene,"中央广场 / 景观湖",new Vector3(110,8,315),18,3);
 box(scene,"办公区停车场",new Vector3(-250,.04,335),new Vector3(260,.05,70),m.path,true,true,false); for(let i=0;i<70;i++){const x=-510+i*15; box(scene,`停车位线-${i}`,new Vector3(x,.08,335),new Vector3(.12,.04,5),m.white,false);} for(let i=0;i<24;i++)createVehicle(scene,`停车场车辆-${i}`,new Vector3(-365+(i%12)*19,0,318+Math.floor(i/12)*28),m,"car",Math.PI/2);
